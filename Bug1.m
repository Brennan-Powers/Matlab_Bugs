...
classdef Bug1 < Navigation
    ...
    properties(Access=protected)
        ...
        edge    % edge list
        k       % edge index
        op      % path fully circling obstacle
        c       % closest point to goal on op so far
    end
...
%                bug.mline = bug.mline / norm(bug.mline(1:2));
                ...
                if bug.isoccupied(robot + [dx; dy])
                    ...
                    bug.op = robot;  % might need to be op=n;
            ...
            if bug.step == 2
                ...
                %----------------------------------------------------------------
                % Inside the else important to bug1
                if bug.k <= numcols(bug.edge)
                    ...
                    bug.step = 3;
                    %op = n;  % might be better if op=robot, check when working
                ...
                bug.op = [bug.op robot(:)];
                %path = [path robot(:)];  (path append code from way above)
                
                dc = sqrt( ((bug.goal(1) - bug.c(1))^2) + ((bug.goal(2) - bug.c(2))^2));
                dr = sqrt( ((bug.goal(1) - robot(1))^2) + ((bug.goal(2) - robot(2))^2));
                if dr < dc
                    bug.c = robot;
                end
            ...
            if bug.step == 3
                ...
                % so now has circled object and back at orgin
                % op is path around the obstacle
                % need to calculate closest point in op to goal
                
                %compute Euclidean distances:
                %distances = sqrt(sum(bsxfun(@minus, bug.goal, bug.op).^2,2));
                
                %find the smallest distance and use that as an index into B:
                %closest = bug.goal(find(distances==min(distances)),:);
                
                % need to move but to closest point
                
                if robot(1) == bug.c(1) && robot(2) == bug.c(2)
                    bug.step = 1;
                    bug.j = bug.j + 1;
